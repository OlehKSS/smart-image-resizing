...
%Function that deletes horizontal "seams" in image. Pass to the function
%the original image, marray and narray, which are the coordinates of the
%pixels to be deleted, and the number of horizontal seams, respectively.
%Marray and narray vs should be in the format 1 x N, where N is the number
%of pixels to be deleted. The output is the image with the horizontal seams
%removed.

function new_image = delete_horizontal_seams(image, narray, marray, num_horiz_seams)
    ...
    new_image = reshape(flattened_image,[height,width,depth]);
